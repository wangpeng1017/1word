...
async function main() {
  const phones = ['13099990004', '13099990006'];
  ...
  console.log('\n' + '='.repeat(70));
  console.log('学生学习单词统计 - ' + todayStr + ' (今天)');
  ...
  for (const phone of phones) {
    ...
    for (const planClass of planClasses) {
      ...
      console.log('   计划单词总数: ' + planClass.vocabulary_packs.totalWords + ' 个');
    }
    ...
    for (const planClass of planClasses) {
      ...
      if (dayDiff >= 0 && dayDiff < planClass.vocabulary_packs.totalDays) {
        ...
        if (packDay) {
          newWordsToday += packDay.wordCount;
        }
      }
    }

    // 6. 今天需要复习的单词
    const reviewToday = await prisma.study_plans.findMany({
      where: {
        studentId: student.id,
        status: 'LEARNING',
        nextReviewAt: {
    ...
    console.log('\n📅 今天具体任务 (Day ' + todayDayNumber + '):');
    console.log('   ✨ 新学: ' + newWordsToday + ' 个');
    console.log('   🔄 复习: ' + reviewToday.length + ' 个');
    console.log('   🎯 今天总计: ' + (newWordsToday + reviewToday.length) + ' 个');
  ...
  for (const phone of phones) {
    ...
    for (const planClass of planClasses) {
      ...
      if (dayDiff >= 0 && dayDiff < planClass.vocabulary_packs.totalDays) {
        ...
        if (packDay) totalNewToday += packDay.wordCount;
      }
    }

    // 今天复习
    const reviewTod = await prisma.study_plans.findMany({
      where: {
        studentId: student.id,
        status: 'LEARNING',
        nextReviewAt: {
    ...
    totalReviewToday += reviewTod.length;
  }
  ...
  console.log('\n📅 今天具体任务 (两个学生合计):');
  console.log('   ✨ 新学: ' + totalNewToday + ' 个');
  console.log('   🔄 复习: ' + totalReviewToday + ' 个');
  console.log('   🎯 今天总计: ' + (totalNewToday + totalReviewToday) + ' 个');
