...
async function main() {
    console.log('Starting comprehensive audio verification...')
    ...
    const hashMap = new Map<string, Array<{ word: string, accent: string, filename: string }>>()
    ...
    if (legacyPaths > 0 || missingFiles > 0 || duplicatesFound > 0) {
        ...
        if (legacyPaths > 0) console.log('1. Delete records with legacy paths (run cleanup-bad-audios.ts)')
        if (duplicatesFound > 0) console.log('2. Delete duplicate/incorrect audio files.')
        console.log('3. Re-run download-word-audios.ts to regenerate correct files.')
    } else {
