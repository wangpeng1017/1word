...
| 日期 | Commit | 改动摘要 | 涉及文件 |
|------|--------|---------|---------| 
| **2026-03-02** | `f2f38d4` | **补打卡支持新词+复习词**: missed day 改为 `mode=all`，弹窗提示更新 | `index.js` |
...
#### 本次快速回滚指令（2026-03-02 部署）

如果本次部署后出现问题，在 XDF 服务器执行：

```bash
cd ~/apps/1word/web-admin
git reset --hard 2528c80
npm run build
pm2 restart word-app
```
...
| 日期 | 版本 | 变更内容 |
|------|------|----------|
| **2026-03-02** | **v2.0** | **91天复习系统上线**: 新增课表引擎、Timeline 91天、补打卡新词+复习词；新增小程序发版记录（补打卡升级）；更新快速回滚指令 |
| 2026-02-27 | v1.5 | 新增 5.4 版本部署记录（修复导出500错误和KPI硬编码） |
