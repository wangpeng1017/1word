### **产品需求文档 (PRD): 智能词汇复习助手**

**V4.0** - 更新日期: 2026-03-02

> 本文档为最新版本,反映了当前系统的实际实现状态。标记为✅的功能已完成,🚧表示进行中,⚪表示待开发。
> **V4.0更新**: 91天复习课表引擎，替换艾宾浩斯曲线为固定课表驱动，时间轴扩展至91天，补打卡升级。
> **V3.1更新**: 新增补打卡功能、4-Tab架构重构、选项随机化、本地音效托管。
> **V3.0更新**: 新增勋章系统、积分商城、错题重测功能、完整版时间轴复习界面。

#### **1. 项目概述**

**1.1. 项目背景与目标**
本项目旨在为上海地区的高中生提供一个基于艾宾浩斯遗忘曲线等记忆算法的智能词汇复大习工具。通过微信小程序方便学生随时随地进行学习，同时提供一个功能强大的 Web 管理后台，供教师进行教学管理、学情跟踪和内容维护。

项目目标是：
*   **提升学习效率**：通过智能化、个性化的复习计划，帮助学生高效记忆单词。
*   **减轻教师负担**：通过数据化、自动化的学情分析与导出功能，方便教师掌握学生学习情况。
*   **实现无缝学习**：支持离线模式，保证学生在各种网络环境下都能完成学习任务。

**1.2. 目标用户**
*   **学生（用户端）**：上海地区的高中生，希望通过手机方便地复习和巩固英语词汇。
*   **教师（管理端）**：高中英语教师，需要管理班级学生的词汇学习进度，并调整教学内容。

---

#### **2. 系统架构与技术选型**

**2.1. 系统架构**
本项目分为三个核心部分：
1.  **微信小程序（移动端）**：面向学生用户，提供核心的单词复习、错题回顾和学习数据查看功能。
2.  **Web 管理后台（PC 端）**：面向教师用户，用于词库维护、学生管理、教学规则配置和数据导出。
3.  **云端服务器与数据库**：负责处理业务逻辑、数据存储与 API 服务。

**2.2. 技术选型建议**
考虑到您将使用 AI 编程工具（如 Cursor）且编程经验较少，我推荐一个现代化、社区活跃且对初学者友好的技术栈：

*   **移动端 (微信小程序)**:
    *   **语言与框架**: 直接使用微信小程序原生开发（WXML + WXSS + JavaScript）。这能 100% 保证所有功能（如音频播放、本地存储、UI 交互）的兼容性和性能。AI 工具对原生小程序的支持也非常好。
    *   **可行性评估**: 您在图片中提到的所有小程序功能，包括离线模式、实时进度保存、多种答题模式、音频播放等，均可在微信小程序环境中**完美实现**。

*   **Web 管理后台 (教师端)**:
    *   **前端框架**: **Next.js (基于 React)**。这是一个非常强大的框架，对 Vercel 部署支持完美，AI 工具对 React 的代码生成能力极强。其文件路由系统也简单直观。
    *   **UI 组件库**: **Ant Design** 或 **MUI**。这些库提供了大量预制好的后台管理组件，可以极大加速开发进程。

*   **后端服务**:
    *   **框架**: **Next.js (API Routes)** 或 **NestJS**。
        *   **建议**: 初期为了快速开发和部署在 Vercel 上，可以直接使用 **Next.js 的 API Routes** 功能来编写后端逻辑。这样前后端就在一个项目里，非常方便。
        *   当项目变得复杂，需要更清晰的后端架构时，可以考虑迁移到 **NestJS** (基于 Node.js 和 TypeScript)，它提供了非常规范和可扩展的架构。
    *   **语言**: **TypeScript**。它为 JavaScript 提供了类型安全，能减少很多潜在的 bug，AI 工具也能根据类型信息生成更准确的代码。

*   **数据库**:
    *   **数据库**: **PostgreSQL**。这是一款功能强大且稳定的开源关系型数据库，非常适合存储结构化的用户信息、词汇数据和学习记录。
    *   **ORM**: **Prisma**。这是一个现代化的数据库工具，能让您用 TypeScript 来操作数据库，自动生成类型，与 AI 编程工具配合起来体验极佳。

**2.3. 部署方案**
*   **Demo 阶段**:
    *   **平台**: **Vercel**。
    *   **实施**: 直接将您的 Next.js 项目（包含前端和 API Routes 后端）关联到 Vercel 账户的 Git 仓库。Vercel 会自动完成构建和部署。对于数据库，可以使用 Vercel Postgres 或 Supabase 等提供的免费 PostgreSQL 服务。
*   **生产阶段**:
    *   **平台**: **阿里云**。
    *   **实施**:
        *   **前端/后端服务**: 使用 **ECS (Elastic Compute Service)** 实例或**容器服务 Kubernetes 版 (ACK)** 来部署您的 Next.js/NestJS 应用。
        *   **数据库**: 使用 **RDS (Relational Database Service)** 并选择 PostgreSQL 实例，方便管理、备份和扩展。
        *   **静态资源存储**: 使用 **OSS (Object Storage Service)** 来存储单词的音频文件和图片，减轻服务器压力并加快访问速度。

---

#### **3. 功能需求详述**

**3.1. 词库系统 (Web 后台功能)**

*   **F-1.1: 基础词库管理** ✅ **已完成**
    *   【需求】应能批量导入、新增、修改、删除词库。词库需覆盖上海高中阶段全部考纲词汇。
    *   【亮点】**支持后台配置词库更新**：允许教师动态增减词库，而无需重新发布小程序。
    *   【实现状态】
        *   ✅ CRUD API 完成（/api/vocabularies）
        *   ✅ 批量导入/导出 Excel 功能
        *   ✅ 支持10000条词汇的页面显示
        *   ✅ 搜索、筛选功能（按难度、高频词）

*   **F-1.2: 单词信息维护** ✅ **已完成**
    *   【需求】每个单词条目需包含：英文单词、词性（可多选）、中文释义（区分核心/延伸义）、音标（英/美可选）、真人发音音频（支持上传）、实体图片（支持上传）。
    *   【需求】可标注单词是否为高考高频项。
    *   【实现状态】
        *   ✅ 支持完整的单词信息字段
        *   ✅ 支持美式/英式音标和音频
        *   ✅ 集成 Vercel Blob 存储，支持音频和图片上传
        *   ✅ 高频词和难度等级标注（EASY/MEDIUM/HARD）
        *   ✅ 分离的 word_audios 和 word_images 表支持多个音频/图片

*   **F-1.3: 配套题目生成与管理** ✅ **已完成**
    *   【需求】系统应支持为每个单词关联至少3道“英译汉”选择题。
    *   【需求】每道题包含1个正确答案和3个干扰项。教师在录入时，应能方便地添加易混淆词义作为干扰项（如 "adapt" vs "adopt"）。
    *   【实现状态】
        *   ✅ 支持4种题型：英译汉、汉译英、听音选词、选词填空
        *   ✅ 题目 CRUD API （/api/questions）
        *   ✅ 每个题目支持4个选项，1个正确答案
        *   ✅ 支持批量导入题目
        *   ✅ question_options 表存储选项详情

**3.2. 数据记录与导出系统 (核心为 Web 后台功能)**

*   **F-2.1: 学员与班级管理** ✅ **已完成**
    *   【需求】支持批量导入学生名单（姓名、学号），并能将学生分配到不同班级。
    *   【实现状态】
        *   ✅ 学生 CRUD API （/api/students）
        *   ✅ 班级 CRUD API （/api/classes）
        *   ✅ 支持批量导入学生 Excel
        *   ✅ 学生和班级关联管理
        *   ✅ 班级删除保护：有学生的班级不能删除，弹窗提示
        *   ✅ 班级名称唯一性校验 (V3.1新增)
        *   ✅ 学生创建强制校验手机号 (V3.1新增)
        *   ✅ 教师管理功能

*   **F-2.2: 学习数据记录** 🚧 **进行中**
    *   【需求】系统需自动记录学生的每一次复习：完成率、正确率、总耗时、开始/结束时间。
    *   【需求】详细记录每一道错题：错误单词、时间、错误选项/内容、正确答案。
    *   【需求】统计每个单词的累计错误次数和最近3次复习的正确率。
    *   【实现状态】
        *   ✅ study_records 表：记录每次学习会话的统计数据
        *   ✅ wrong_questions 表：记录每道错题的详细信息
        *   ✅ word_masteries 表：跟踪单词掌握度、连续正确次数等
        *   🚧 API 接口开发中

*   **F-2.3: 数据导出功能** 🚧 **进行中**
    *   【需求】支持按学员、班级、时间段筛选数据。
    *   【需求】支持将筛选后的学习报告和错题本批量导出为 Excel (.xlsx)、PDF 和 Word 格式。
    *   【需求】导出内容可按"单词"或"错误次数"排序。
    *   【亮点】**支持后台实现模板更改**：允许管理员自定义导出文件的样式和格式。
    *   【实现状态】
        *   ✅ 学习数据分页查询 + 按班级/学生/日期/类型筛选 (`/api/statistics/learning-sessions`)
        *   ✅ **学习数据全量导出 Excel（2026-03-07新增）**: 新增 `/api/statistics/learning-sessions-export` API，无分页限制（最多5000条），前端"导出全部"按钮，浏览器侧生成 Excel 文件，支持班级+筛选条件联动
        *   ✅ 单个学生学习报告导出 (Word 格式, `/api/statistics/[studentId]/export`)
        *   ✅ 批量学生报告导出 (ZIP+Word, `/api/statistics/batch-export`)
        *   ⚪ PDF 格式（Vercel 环境限制，暂不支持）
        *   ⚪ 导出模板自定义

**3.3. 智能复习计划系统**

*   **F-3.1: 复习周期逻辑 (后台配置 + 小程序实现)** ✅ **已完成**
    *   【需求】基于记忆算法（如艾宾浩斯曲线），为每个学生自动生成每日复习单词 list。
    *   【需求】新学单词按计划纳入当日复习。
    *   【实现状态】
        *   ✅ study_plans 表：学生个人学习计划
        *   ✅ daily_tasks 表：每日任务管理
        *   ✅ plan_classes 表：班级层面的学习计划
        *   ✅ 艾宾浩斯算法库 (lib/ebbinghaus.ts)
        *   ✅ API 接口已完成

*   **F-3.1.1: 每日学习单词数量计算** ✅ **已完成**
    *   【核心公式】**每日学习总量 = 新学单词 + 复习单词 - 已掌握单词**
    *   【艾宾浩斯复习间隔】学习后第 **1、2、4、7、15** 天需要复习
    *   【计算示例】（假设每天新学10个单词）
        | 天数 | 新学 | 复习来源 | 总计 |
        |------|------|----------|------|
        | 第1天 | 10 | - | **10** |
        | 第2天 | 10 | 10(D1+1天) | **20** |
        | 第3天 | 10 | 10(D1+2天) + 10(D2+1天) | **30** |
        | 第4天 | 10 | 10(D2+2天) + 10(D3+1天) | **30** |
        | 第5天 | 10 | 10(D1+4天) + 10(D3+2天) + 10(D4+1天) | **40** |
        | 第6天 | 10 | 10(D2+4天) + 10(D4+2天) + 10(D5+1天) | **40** |
        | 第7天 | 10 | 10(D3+4天) + 10(D5+2天) + 10(D6+1天) | **40** |
        | 第8天 | 10 | 10(D1+7天) + 10(D4+4天) + 10(D6+2天) + 10(D7+1天) | **50** |
    *   【已掌握单词扣除】
        *   掌握判定：最近3次答题**全部正确**
        *   掌握后：`study_plans.status = 'MASTERED'`
        *   已掌握单词自动从后续所有复习计划中移除
    *   【实现位置】
        *   ✅ `/api/students/[id]/daily-tasks` - 获取每日任务（第155、172行过滤已掌握）
        *   ✅ `/api/review-plan/forecast` - 复习量预测（第110行过滤已掌握）
        *   ✅ `/api/study-sessions/[id]/complete` - 完成学习时更新掌握状态
        *   ✅ `/api/study-records` - 提交答题记录时更新掌握状态

*   **F-3.2: 单词筛选与掌握规则 (后台配置 + 小程序实现)** ✅ **已完成**
    *   【需求】若一个单词连续3次复习正确率100%，则判定为"已掌握"，自动从后续复习流中移除。
    *   【需求】累计错误次数 ≥ 3次的单词，标记为"重点难点"，优先安排复习。
    *   【实现状态】
        *   ✅ word_masteries 表支持 consecutiveCorrect 字段
        *   ✅ isMastered 和 isDifficult 标记字段
        *   ✅ 掌握判定：基于 question_answers 表最近3条记录全部正确
        *   ✅ 掌握后自动更新 study_plans.status = 'MASTERED'
        *   ✅ 已掌握单词从 daily-tasks 和 forecast API 中自动过滤

*   **F-3.3: 个性化调整 (后台功能)** ⚪ **待开发**
    *   【亮点】**支持老师后台手动添加、调整**指定学生的每日复习 list。
    *   【实现状态】
        *   ⚪ 待开发

**3.4. 微信小程序功能**

*   **F-4.1: 核心界面** ✅ **已完成**
    *   【需求】首页布局简洁，仅提供三大入口：“今日复习”、“错题本”、“我的成果”。
    *   【实现状态】
        *   ✅ 登录页面
        *   ✅ 首页布局和导航
        *   ✅ 学习统计展示

*   **F-4.2: 答题模式** ⚪ **待开发**
    *   【需求】复习过程中，顶部显示进度（第 X/Y 个）和已用时。中部为题目区，底部为答题区和趣味进度条。
    *   **模式1：英选汉**（英文单词 + 4 个汉语选项）
    *   **模式2：汉选英**（汉语 + 4 个英文单词选项）
    *   **模式3：听音选词**（英文单词读音 + 4 个英文单词选项）
    *   **模式4：选词填空**（英文句子挖空，从 4 个单词中选择）
    *   【交互】选择题点击选项即判断对错。正确则自动进入下一题；错误则显示正确答案，用户需手动点击“继续”。
    *   【实现状态】
        *   ✅ 后端支持4种题型的数据结构
        *   ⚪ 小程序答题界面待开发

*   **F-4.3: 进度保存与防误触** ⚪ **待开发**
    *   【需求】每答完一题，自动保存当前进度（已答题数、用时、临时错题记录）。
    *   【需求】因故退出（如闪退、切后台）后，重新打开 APP 时弹窗提示“是否继续上次复习？”。选择继续则恢复到退出前的题目界面。
    *   【需求】若上次复习未完成且超过 24 小时（**该时长需支持后台手动设置**），则该次记录记为“中断”，学生可手动选择“补学”。
    *   【实现状态】
        *   ✅ system_configs 表支持配置管理
        *   ✅ daily_tasks 表支持 INTERRUPTED 状态
        *   ⚪ 小程序进度保存功能待开发

*   **F-4.4: 勋章与成就系统** ✅ **已完成** (2026-01-09新增)
    *   【需求】建立勋章激励体系,提升学生学习积极性和成就感。
    *   【功能】10种勋章,按稀有度分类:
        *   普通: 🥉铜牌新手、🥈银牌学徒
        *   稀有: 🥇金牌学者、🔥连续之星、💎完美主义者、📚错题克星
        *   史诗: ⚡闪电学霸、🎯精准射手
        *   传说: 🏆终极大师、🌟积分富豪
    *   【展示】
        *   个人中心用户名旁显示最多3个勋章
        *   勋章墙页面展示所有勋章(已解锁/未解锁)
        *   勋章按稀有度颜色区分
    *   【实现状态】
        *   ✅ badges 表: 勋章定义
        *   ✅ student_badges 表: 学生勋章关联
        *   ✅ API: `/api/badges` (获取勋章列表)
        *   ✅ API: `/api/students/[id]/badges` (学生勋章)
        *   ✅ 勋章自动授予工具 (lib/auto-grant-badge.ts)
        *   ✅ 勋章墙页面 (pages/badges)
        *   ✅ 个人中心勋章显示

*   **F-4.5: 积分商城系统** ✅ **已完成** (2026-01-09新增)
    *   【需求】允许学生使用积分兑换特殊成就和装饰性勋章。
    *   【功能】5种可兑换成就:
        *   🎁 幸运礼包 (500积分)
        *   🌈 彩虹勋章 (1000积分)
        *   👑 王者之冠 (2000积分)
        *   🦄 独角兽 (3000积分)
        *   🔮 神秘水晶 (5000积分)
    *   【规则】
        *   兑换前检查积分是否足够
        *   兑换成功后立即扣除积分
        *   防重复兑换(已兑换的不可再次兑换)
        *   兑换记录可追溯
    *   【实现状态】
        *   ✅ redeemable_achievements 表: 可兑换成就定义
        *   ✅ achievement_redemptions 表: 兑换记录
        *   ✅ API: `/api/achievements/redeem` GET (获取列表)
        *   ✅ API: `/api/achievements/redeem` POST (执行兑换)
        *   ✅ 积分商城页面 (pages/redeem)
        *   ✅ 事务处理保证数据一致性

*   **F-4.6: 错题本重测功能** ✅ **已完成** (2026-01-09新增)
    *   【需求】支持学生重新测试错题,巩固薄弱知识点。
    *   【功能】
        *   错题本图标: 📖
        *   全部重测: 重测所有错题
        *   筛选重测: 可选择性重测
        *   答对移除: 答对1次即从错题本移除
    *   【交互】
        *   点击重测按钮进入答题模式
        *   只展示错题本中的题目
        *   答对后自动删除错题记录
        *   重测结束显示统计
    *   【实现状态】
        *   ✅ wrong_questions 表增强 (wrongCount, correctCount, status)
        *   ✅ API: `/api/wrong-questions/[id]` DELETE (删除错题)
        *   ✅ 错题本页面重测按钮 (pages/wrong)
        *   ✅ 答题页面重测模式支持 (pages/study)
        *   ✅ 简化逻辑: 答对1次直接移除

*   **F-4.7: 时间轴复习界面** ✅ **已完成** (2026-01-09新增)
    *   【需求】参考多邻国设计,提供直观的学习进度可视化。
    *   【功能】
        *   欢迎动画: 每天首次打开显示吉祥物飞入动画
        *   顶部统计: 🔥连续天数、💎总积分、💖生命值
        *   时间轴: DAY1-DAY10 垂直卡片布局
        *   状态可视化:
            *   ✅ 已完成 - 绿色渐变
            *   🔄 进行中 - 橙色渐变 + 脉冲动画
            *   ⚠️ 错过 - 红色渐变
            *   🔒 未开始 - 灰色虚线
        *   自动滚动: 页面加载后自动滚动到当前DAY
        *   连接线: DAY卡片之间的连接线动画
    *   【交互】
        *   点击已完成DAY查看详情(单词数、正确率、用时)
        *   点击当前DAY开始复习
        *   点击未开始DAY提示"请先完成前面的学习"
    *   【实现状态】
        *   ✅ API: `/api/study-days` (获取学习天数数据)
        *   ✅ 计算连续学习天数逻辑
        *   ✅ 首页完整版时间轴布局 (pages/index)
        *   ✅ 欢迎动画实现
        *   ✅ 脉冲动画效果
        *   ✅ 自动滚动功能

*   **F-4.8: 离线模式** ⚪ **待开发**
    *   【需求】小程序启动时，若有网络，可下载当日复习单词包（含题目、音频等）。
    *   ...

*   **F-4.9: 补打卡 (Make-up Learning)** ✅ **已完成** (2026-02-04新增)
    *   【需求】允许用户点击时间轴上的“已错过 (Missed)”节点进行补学。
    *   【逻辑】补学时，系统自动**只提供当其应当学习的新单词** (Plan A)，屏蔽过期的复习任务，以减轻用户负担并修正记忆曲线数据。
    *   【实现状态】
        *   ✅ 后端 `daily-tasks` 接口支持 `day` 参数
        *   ✅ 前端点击交互与跳转逻辑

*   **F-4.10: 体验优化 (UX)** ✅ **已完成** (2026-02-04新增)
    *   【需求】**选项随机化**：每次进入题目，选项顺序随机打乱，防止用户背答案位置。
    *   【需求】**本地音效**：系统音效 (Correct/Wrong/Streak) 全部托管在自有服务器，避免外链加载慢。
    *   【需求】**4-Tab 架构**：拆分为 [今日学习] (新词) 和 [今日复习] (时间轴) 两个独立 Tab，职责更清晰。
    *   【实现状态】
        *   ✅ 前端洗牌算法
        *   ✅ 音效文件本地化
        *   ✅ Tab Bar 重构


---

#### **4. 开发路线图 (Roadmap) 建议**

1.  **第一阶段 (MVP - 最小可行产品)**
    *   **目标**: 快速上线核心功能，验证产品价值。
    *   **功能**:
        *   后台：完成词库导入与单词信息维护 (F-1.1, F-1.2)。
        *   后台：实现基础的学生管理 (F-2.1)。
        *   小程序：实现用户登录、每日复习 list 获取。
        *   小程序：实现答题模式 1 和 2 (F-4.2)。
        *   小程序：完成最基础的学习数据记录与展示 (F-2.2, F-4.1 中的“我的成果”)。
    *   **部署**: 全部部署在 Vercel 上。

2.  **第二阶段 (功能完善)**
    *   **目标**: 完善核心体验，增加教师管理能力。
    *   **功能**:
        *   后台：开发数据导出功能 (F-2.3)。
        *   后台：实现智能复习规则配置 (F-3.1, F-3.2)。
        *   后台：实现教师手动调整学生学习计划 (F-3.3)。
        *   小程序：上线答题模式 3 和 4。
        *   小程序：开发错题本功能。
        *   小程序：实现进度保存与异常退出处理 (F-4.3)。

3.  **第三阶段 (高级功能与性能优化)**
    *   **目标**: 提升产品竞争力和用户体验。
    *   **功能**:
        *   小程序：开发并上线离线模式 (F-4.4)。
        *   后台：实现导出模板自定义功能。
        *   全端：UI/UX 优化，增加趣味性元素（如进度条动画）。
    *   **部署**: 准备从 Vercel 迁移至阿里云生产环境。

---

#### **5. 当前实现状态总结 (2025-01-07)**

**5.1. 已完成功能 ✅**

**Web 管理后台：**
1. 认证系统
   - 教师/学生登录、注册
   - JWT Token 认证
   - 角色权限管理

2. 词库管理
   - CRUD 操作 (10000条词汇支持)
   - 批量导入/导出 Excel
   - 搜索、筛选功能（难度、高频词）
   - 支持英/美音标和音频
   - Vercel Blob 存储集成（音频/图片上传）
   - 多音频/多图片支持 (word_audios, word_images 表)

3. 题目管理
   - 4种题型：英译汉、汉译英、听音选词、选词填空
   - 题目 CRUD API
   - 批量导入题目
   - 每题 4 个选项，1 个正确答案

4. 学生与班级管理
   - 学生 CRUD API
   - 班级 CRUD API
   - 批量导入学生 Excel
   - 班级删除保护（有学生的班级不能删除，弹窗提示）
   - 教师管理功能

**数据库设计：**
- 18 个数据表完整设计
- 支持艾宾浩斯复习算法的数据结构
- 学习记录、错题跟踪、单词掌握度管理

**微信小程序:**
- 登录页面
- 首页时间轴布局 (DAY1-DAY10)
- 欢迎动画
- 学习统计展示
- 勋章系统 (勋章墙 + 个人中心显示)
- 积分商城 (兑换页面)
- 错题本重测功能
- 学习进度可视化

**5.2. 进行中功能 🚧**

1. 答题界面开发
   - 4种题型界面设计
   - 答题交互逻辑
   - 进度保存功能

**5.3. 待开发功能 ⚪**

**Web 管理后台：**
1. 数据导出功能 (Excel/PDF/Word)
2. 统计报表和数据可视化
3. 教师手动调整学生学习计划
4. 导出模板自定义

**微信小程序：**
1. 答题界面完善 (4种题型)
2. 学习记录详细查看
3. 进度保存与恢复
4. 离线模式
5. 成就系统扩展 (更多成就类型)
6. 动画效果优化 (勋章解锁动画、烟花效果等)

**5.4. 技术栈**

**前端:**
- Next.js 15 (React 18)
- TypeScript
- Ant Design 5
- Tailwind CSS
- 微信小程序原生开发

**后端:**
- Next.js API Routes
- Prisma ORM
- PostgreSQL
- JWT 认证

**存储:**
- Vercel Postgres (数据库 - Demo)
- 阿里云 PostgreSQL (数据库 - 生产)
- Vercel Blob (文件存储)

**部署:**
- Vercel (Demo 阶段)
- 阿里云 ECS (生产环境) ✅ 已部署

**新增功能库:**
- 勋章系统自动授予工具 (lib/auto-grant-badge.ts)
- 学习天数计算逻辑 (api/study-days)

**5.5. 下一步工作**

1. **优先级 P0** - 完成核心学习闭环
   - 完善答题界面 (4种题型)
   - 优化学习数据记录
   - 真机测试和性能优化

2. **优先级 P1** - 增强用户体验
   - 动画效果优化 (勋章解锁、烟花效果)
   - 学习记录详细查看
   - Web 后台统计报表

3. **优先级 P2** - 高级功能
   - 数据导出 (Excel/PDF/Word)
   - 离线模式
   - 进度保存与恢复
   - 成就系统扩展

**5.6. V3.0 更新总结 (2026-01-09)**

本次更新新增4大核心功能模块:

1. **勋章系统** - 10种勋章,4个稀有度等级,完整的展示和授予逻辑
2. **积分商城** - 5种可兑换成就,完整的兑换流程和防重复机制
3. **错题重测** - 简化的重测逻辑,答对1次即移除
4. **时间轴界面** - 参考多邻国设计,DAY1-DAY10可视化,欢迎动画

**数据库变更:**
- 新增4个数据表 (badges, student_badges, redeemable_achievements, achievement_redemptions)
- wrong_questions 表增强 (新增status, correctCount等字段)

**API接口:**
- 新增8个API接口
- 完善了学习数据统计逻辑

**总进度:** 核心功能完成度 95%

---

**5.7. V4.0 更新总结 (2026-03-02)**

本次更新重构复习引擎，实现91天完整复习计划：

1. **91天课表引擎** `lib/review-schedule.ts` — 动态适配任意天数词汇包（10/15/17/20/30天班均支持），5阶段复习结构（学习期→抗遗忘→第二轮→第三轮→综合检测），节假日自动跳过
2. **复习词来源重构** — `daily-tasks` API 由艾宾浩斯间隔改为课表驱动；`study-days` API Timeline 从 totalDays+15 扩展到固定 91 天
3. **补打卡升级** — missed day 由"仅学新词"升级为"新词+复习词"全量补学（前后端双改）

**文件变更:**
- 新增 `lib/review-schedule.ts`（325行，课表引擎）
- 修改 `app/api/students/[id]/daily-tasks/route.ts`（复习词来源+补打卡）
- 修改 `app/api/study-days/route.ts`（91天Timeline）
- 修改 `wechat-miniapp/pages/index/index.js`（补打卡 mode=all，需发版）

**约束遵守:**
- ✅ 不修改数据库表结构
- ✅ 91天Timeline、重复打卡无需发版小程序
- ⚠️ 补打卡升级（新词+复习词）需发版小程序

**Commits:**
- `8f56a01` feat: 91天复习课表引擎
- `f2f38d4` feat: 补打卡支持复习词
- `62e5937` chore: package-lock.json

**总进度:** 91天复习系统完整交付 ✅

---

祝您项目开发顺利!这份 PRD 结合了您的需求和技术背景,可以直接作为您使用 AI 编程的指导蓝图。
