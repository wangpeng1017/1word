...
### Added
- **Mobile (Uni-app)**: 完成从微信小程序到 Uni-app 的全面迁移，支持 Android 和 iOS。
  - **核心页面**: 迁移了首页(Timeline)、今日学习、错题本、个人中心、登录页。
  - **答题引擎**: 适配 4 种题型，实现原生音频预加载与物理返回键拦截。
  - **同步机制**: 增强了断网重连后的进度同步逻辑。
  - **UX 优化**: 全局适配沉浸式状态栏与原生振动反馈。
